...
(JNIEnv *env, jobject jobj, jfloatArray img_, jfloatArray M_, jfloatArray O_, jint nrows_img, jint ncols_img, jint nchannels_img, jboolean full_angle)
{
	jni_wrap jw(env, jobj);
	std::vector<float> img = jw.from_jfloatArray(img_);
	std::vector<float> M(env->GetArrayLength(M_));
	std::vector<float> O(env->GetArrayLength(O_));
	gradMag(img.data(), M.data(), O.data(), nrows_img, ncols_img, nchannels_img, full_angle);
	jw.set_jfloatArray_inputArg(M, M_);
	jw.set_jfloatArray_inputArg(O, O_);
}
...
(JNIEnv *env, jobject jobj, jfloatArray M_, jfloatArray O_, jfloatArray H_, jint nrows_img, jint ncols_img, jint binSize, jint nOrients, jint softBin, jboolean full_angle)
{	
	jni_wrap jw(env, jobj);
	std::vector<float> M = jw.from_jfloatArray(M_);
	std::vector<float> O = jw.from_jfloatArray(O_);
	std::vector<float> H(env->GetArrayLength(H_));
	gradHist(M.data(), O.data(), H.data(), nrows_img, ncols_img, binSize, nOrients, softBin, full_angle);
	jw.set_jfloatArray_inputArg(H, H_);
}
...
(JNIEnv *env, jobject jobj, jfloatArray M_, jfloatArray O_, jfloatArray H_, jint nrows_img, jint ncols_img, jint binSize, jint nOrients, jint softBin, jboolean full_angle, jfloat clipHog)
{
	jni_wrap jw(env, jobj);
	std::vector<float> M = jw.from_jfloatArray(M_);
	std::vector<float> O = jw.from_jfloatArray(O_);
	std::vector<float> H(env->GetArrayLength(H_));
	hog(M.data(), O.data(), H.data(), nrows_img, ncols_img, binSize, nOrients, softBin, full_angle, clipHog);
	jw.set_jfloatArray_inputArg(H, H_);
}
...
(JNIEnv *env, jobject jobj, jfloatArray M_, jfloatArray O_, jfloatArray H_, jint nrows_img, jint ncols_img, jint binSize, jint nOrients, jint softBin, jfloat clipHog)
{
	jni_wrap jw(env, jobj);
	std::vector<float> M = jw.from_jfloatArray(M_);
	std::vector<float> O = jw.from_jfloatArray(O_);
	std::vector<float> H(env->GetArrayLength(H_));
	fhog(M.data(), O.data(), H.data(), nrows_img, ncols_img, binSize, nOrients, softBin, clipHog);
	jw.set_jfloatArray_inputArg(H, H_);
}
